...
export const POST = async () => {
  try {
    ...
    const youtubeVideoURL = "https://www.youtube.com/watch?v=cFHDE63QDoY"
    ...
    const chooseVideoFormat = () => {
      ...
      const chosenVideoFormat = idealVideoFormats.find((format) =>
        itagsFormats.includes(format)
      )
    ...
    const chooseAudioFormat = () => {
      ...
      const chosenAudioFormat = idealAudioFormats.find((format) => 
        itagsFormats.includes(format)
      )
    ...
    const ffmpegProcessPromise = new Promise((resolve, reject) => {
      ffmpegProcess.on('close', (code) => {
        ...
        console.log(`Ffmpeg process exited with code ${code} in ${( (tracker.end - tracker.start) / 1000 ).toFixed(2)} seconds.`);
        resolve()
      });
    })
    ...
    ffmpegProcess.stdio[5].pipe(mergeVideoAndAudioStream)

    const audioKeyFilePath = `ytdl/audio/${outputVideoName}.m4a`
    const videoKeyFilePath = `ytdl/${outputVideoName}.mp4`
    const [ signedAudioURL, outputVideoURL ] = await Promise.all([
      uploadStreamToS3(audioStream, audioKeyFilePath),
